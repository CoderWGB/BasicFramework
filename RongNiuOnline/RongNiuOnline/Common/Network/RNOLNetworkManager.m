...
- (AFHTTPSessionManager *)httpManager{
    if (!_httpManager) {
        ...
        _httpManager.requestSerializer.timeoutInterval = 30; //30s超时
    }
    return _httpManager;
}
...
- (void)GET:(NSString *)url parameters:(NSDictionary *)params complationHandle:(void (^)(id, NSError *))completed{
	BOOL isCache = params[@"allowRequestCache"];
	if(isCache){//需要缓存网络请求
		[PPNetworkHelper GET: url parameters: params responseCache:^(id responseCache) {
			completed(responseCache, nil);
...
- (void)uploadFileWithURL:(NSString *)url
    ...
    [self.httpManager POST: url parameters: params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        NSLog(@"%.2lf%%", progress);
        NSString * progressStr = kWGB_STR(@"%.2lf%%", progress);
