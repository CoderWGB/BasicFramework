...
@implementation UIView (RNOLExtension)
...
- (void)addDoubleClickWithBlock:(GestureActionBlock)block{
    
    UITapGestureRecognizer *doubleClickTap = objc_getAssociatedObject(self, &kActionHandlerDoubleTapGestureKey);
    if (!doubleClickTap) {
        doubleClickTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleActionForDoubleClickGesture:)];
        doubleClickTap.numberOfTapsRequired = 2; //双击
        [self addGestureRecognizer: doubleClickTap];
        objc_setAssociatedObject(self, &kActionHandlerDoubleTapGestureKey, doubleClickTap, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    objc_setAssociatedObject(self, &kActionHandlerDoubleTapBlockKey, block, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
}

- (void)handleActionForDoubleClickGesture:(UITapGestureRecognizer *)gesture{
    if (gesture.numberOfTapsRequired == 2) {
        GestureActionBlock block = objc_getAssociatedObject(self, &kActionHandlerDoubleTapBlockKey);
        !block? : block(gesture);
    }
}
